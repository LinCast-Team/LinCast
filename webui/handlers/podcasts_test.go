...
func TestSubscribeToPodcastHandler(t *testing.T) {
	...
	assert.Equal(int64(-1), r.ContentLength, "The response should not have a content") // -1 means that the content length is unknown
	...
	assert.NotEqual(int64(-1), r.ContentLength, "The response should have a content, since the body must contain the description of the error")
}
