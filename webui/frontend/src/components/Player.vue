<template>
<div class="flex flex-col border-2 border-solid transition-colors duration-500 border-gray-100 bg-gray-50 dark:border-gray-700 dark:bg-gray-900 z-100 shadow-lg font-sans" :class="{ 'fixed bottom-0 right-0 left-0 top-0 rounded-t-xl': !expanded }">
  <h1 class="text-2xl my-8 dark:text-gray-300">Playing Now</h1>
  <div class="flex flex-col text-center items-center mb-6">
    ...
    <div class="grid grid-cols-4 grid-flow-col gap-2 items-center justify-items-center my-4 md:my-8 dark:text-gray-100">
      <div class="" data-feather="share-2"></div>
      <div class="col-span-2 text-center">
        <p class="truncate text-black dark:text-gray-100 uppercase font-extrabold text-xl">{{ podcastTitle }}</p>
        <p class="truncate text-gray-500 dark:text-gray-400">{{ episodeTitle }}</p>
      </div>
      <div class="" data-feather="more-vertical"></div>
    </div>
  </div>
  <div id="waveform" class="bg-transparent mx-4"></div>
  <div class="bg-transparent text-black transition-colors duration-500 dark:text-gray-100 py-4 px-1 sm:px-3 lg:px-1 xl:px-3 my-auto md:mx-20 grid grid-cols-5 items-center">
